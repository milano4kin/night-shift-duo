...
DREAD SHIFT содержит оружие и уничтожение вымышленных зомби в стилизованной top-down графике. Для текущей версии разумная отправная точка — **12+**, если в финальном билде нет реалистичной крови, расчленения или натуралистичных сцен. Если визуал станет реалистичнее или жестче, перепроверь категорию 16+/18+ по официальной таблице перед отправкой.

Не ставь 0+ или 6+ автоматически: модерация оценивает реализм мира, оружия и самого насилия.

## Размеры окна Desktop

Перед модерацией обязательно проверь как минимум:
- 1280×1024;
- 1366×768;
- 1600×900;
- 1920×1080;
...
На всех размерах кнопки, тексты и HUD должны оставаться видимыми, не перекрываться и не вызывать системный scrollbar. Текущая игра уже использует `overflow:hidden`, а Yandex-сборка дополнительно блокирует overscroll.
