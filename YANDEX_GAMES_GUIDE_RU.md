...
Дополнительно через `player.setData()` отправляется небольшой cloud snapshot:
- версия;
- silver;
- gold;
- bestWave;
...
Основным источником прогресса остаётся server-side save, потому что DREAD SHIFT имеет серверную экономику и боевую логику.
