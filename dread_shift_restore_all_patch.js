...
(()=>{
  ...
  let client=read("public/client.js");
  let css=read("public/style.css");
  const html=read("public/index.html");
  if(!html.includes('id="notificationBell"'))throw new Error("restore-all: notification bell missing from HTML");
  if(!html.includes('id="openWhatsNewBtn"'))throw new Error("restore-all: What's New button missing from HTML");
  if(!html.includes('id="indexBookBtn"'))throw new Error("restore-all: INDEX button missing from HTML");
  if(!html.includes('id="codesBtn"'))throw new Error("restore-all: CODES button missing from HTML");
  if(!html.includes('id="codesOverlay"'))throw new Error("restore-all: CODES overlay missing from HTML");
  const clientAppend="\n/* DREAD SHIFT restore-all top lobby tools */\n(()=>{\n  const dsRestoreTopUi=()=>{\n    const lobbyEl=document.getElementById(\"lobby\");\n    const authEl=document.getElementById(\"authOverlay\");\n    const bell=document.getElementById(\"notificationBell\");\n    const whats=document.getElementById(\"openWhatsNewBtn\");\n    const side=document.getElementById(\"lobbySideActions\");\n    const show=!!(lobbyEl&&lobbyEl.classList.contains(\"visible\"))&&!(authEl&&authEl.classList.contains(\"visible\"))&&!(typeof state!==\"undefined\"&&state?.started);\n    if(bell)bell.classList.toggle(\"hidden\",!show);\n    if(whats){\n      whats.classList.toggle(\"hidden\",!show);\n      whats.classList.add(\"restore-whats-new\");\n      if(bell?.parentElement&&whats.previousElementSibling!==bell)bell.after(whats);\n    }\n    if(side)side.classList.toggle(\"visible\",show);\n    if(!show){\n      document.getElementById(\"codesOverlay\")?.classList.remove(\"visible\");\n      document.getElementById(\"indexOverlay\")?.classList.remove(\"visible\");\n    }\n  };\n  const previous=typeof syncNotificationUi===\"function\"?syncNotificationUi:null;\n  if(previous)syncNotificationUi=function(){previous();dsRestoreTopUi();};\n  setInterval(dsRestoreTopUi,300);\n  setTimeout(dsRestoreTopUi,0);\n})();\n";
  const cssAppend="\n/* DREAD SHIFT restore-all top lobby tools */\n#notificationBell.notification-bell{position:fixed!important;top:14px!important;right:14px!important;z-index:100!important}\n#openWhatsNewBtn.restore-whats-new{position:fixed!important;top:14px!important;right:66px!important;z-index:100!important;width:auto!important;min-height:42px!important;padding:0 12px!important;border-radius:12px!important;font-size:11px!important;white-space:nowrap!important}\n#notificationCenter.notification-center{position:fixed!important;top:68px!important;right:14px!important;z-index:99!important}\n.lobby-side-actions{z-index:98!important}\n@media(max-width:760px){\n  #notificationBell.notification-bell{top:10px!important;right:10px!important}\n  #openWhatsNewBtn.restore-whats-new{top:10px!important;right:58px!important;min-height:40px!important;padding:0 9px!important}\n  #notificationCenter.notification-center{top:62px!important;right:10px!important}\n}\n";
  if(!client.includes("/* DREAD SHIFT restore-all top lobby tools */")){client+=clientAppend;write("public/client.js",client);}
  if(!css.includes("/* DREAD SHIFT restore-all top lobby tools */")){css+=cssAppend;write("public/style.css",css);}
  const server=read("server.js"),finalClient=read("public/client.js"),finalCss=read("public/style.css");
  const required=[
    ...
    [finalClient.includes("DREAD SHIFT restore-all top lobby tools"),"top lobby UI runtime"],
    [finalCss.includes("DREAD SHIFT restore-all top lobby tools"),"top lobby UI styles"]
  ];
