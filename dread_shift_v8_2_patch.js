...
function rep(s,a,b){return s.includes(a)?s.replace(a,b):s;}
function assert(s,needle,label){if(!s.includes(needle))throw new Error(`v8.2 verification failed: ${label}`);}

function patchServer(){
  let s=read("server.js");
  if(s.includes("/* DREAD SHIFT v8.2 server */"))return false;
  s=rep(s,'const BUILD_VERSION = "8.1.0";','const BUILD_VERSION = "8.2.0";');
  const costPairs=[
  ...
  for(const [a,b] of costPairs)s=rep(s,a,b);
  if(!s.includes('STRUCTURE_MAX_LEVEL_BY_TYPE')){
    s=rep(s,'const STRUCTURE_MAX_LEVEL = 5;','const STRUCTURE_MAX_LEVEL = 5;\nconst STRUCTURE_MAX_LEVEL_BY_TYPE={wall:5,gate:5,cannon:5,tesla:5,cat_tower:3,frost_tower:5,flame_tower:5,spikes:3};\nconst STRUCTURE_LEVEL_HP=[0,1,1.25,1.55,1.90,2.35];\nconst STRUCTURE_LEVEL_POWER=[0,1,1.18,1.40,1.68,1.98];\nconst STRUCTURE_LEVEL_RANGE=[0,1,1,1.06,1.12,1.18];\nconst STRUCTURE_LEVEL_COOLDOWN=[0,1,.97,.93,.88,.82];\nfunction structureMaxLevel(type){return STRUCTURE_MAX_LEVEL_BY_TYPE[type]||STRUCTURE_MAX_LEVEL;}\nfunction structureRangeMultiplier(st){return STRUCTURE_LEVEL_RANGE[Math.max(1,Math.min(5,st?.level||1))]||1;}\nfunction structureCooldownMultiplier(st){return STRUCTURE_LEVEL_COOLDOWN[Math.max(1,Math.min(5,st?.level||1))]||1;}');
  }
  s=s.replaceAll('if(nextLevel > STRUCTURE_MAX_LEVEL) return null;','if(nextLevel > structureMaxLevel(type)) return null;');
  s=s.replaceAll('if(st.level >= STRUCTURE_MAX_LEVEL) return send(p.ws,"notice",{text:"Постройка уже максимального уровня"});','if(st.level >= structureMaxLevel(st.type)) return send(p.ws,"notice",{text:"Постройка уже максимального уровня"});');
  ...
  s=rep(s,'let t=null,best=520;','let t=null,best=520*structureRangeMultiplier(s);');
  s=rep(s,'s.cooldown=1.25;addEffect','s.cooldown=1.25*structureCooldownMultiplier(s);addEffect');
  s=rep(s,'let t=null,best=400;','let t=null,best=400*structureRangeMultiplier(s);');
  s=rep(s,'s.cooldown=1.08;','s.cooldown=1.08*structureCooldownMultiplier(s);');
  s=rep(s,'let t=null,best=125;','let t=null,best=125*structureRangeMultiplier(s);');
  s=rep(s,'s.cooldown=.56;','s.cooldown=.56*structureCooldownMultiplier(s);');
  s=rep(s,'let t=null,best=430;','let t=null,best=430*structureRangeMultiplier(s);');
  s=rep(s,'s.cooldown=.88;','s.cooldown=.88*structureCooldownMultiplier(s);');
  s=rep(s,'const targets=room.zombies.filter(z=>dist(s,z)<185)','const targets=room.zombies.filter(z=>dist(s,z)<185*structureRangeMultiplier(s))');
  s=rep(s,'s.cooldown=.72;','s.cooldown=.72*structureCooldownMultiplier(s);');
  assert(s,'cost:{wood:8,stone:3,scrap:2}','wall price');
  assert(s,'STRUCTURE_MAX_LEVEL_BY_TYPE','structure levels');
...
function patchClient(){
  ...
  const injected=[
    ...
    'function maybeStoreNotification(text){var x=String(text||"");if(/достижен|achievement|нов(ый|ого) питом|new pet|новый враг|new enemy|код .*активирован|code .*redeem|уровень аккаунта|account level|обучение завершено|tutorial completed|босс|boss loot|открыт новый|unlocked/i.test(x))pushNotification(x,"event");}',
    ...
    'function syncLobbyFocusMode(){var join=document.getElementById("joinCodePanel"),wait=document.getElementById("roomWaitPanel");var focus=!!(join&&!join.classList.contains("hidden"))||!!(wait&&!wait.classList.contains("hidden"));document.body.classList.toggle("room-focus-mode",focus);}',
    'setTimeout(function(){var bell=document.getElementById("notificationBell"),panel=document.getElementById("notificationCenter"),clear=document.getElementById("clearNotificationsBtn");if(bell)bell.addEventListener("click",function(){notificationPanelOpen=!notificationPanelOpen;if(panel)panel.classList.toggle("visible",notificationPanelOpen);if(notificationPanelOpen){uiNotifications=uiNotifications.map(function(n){n.read=true;return n});storageSet("dread_notifications",JSON.stringify(uiNotifications));renderNotifications();}});if(clear)clear.addEventListener("click",function(){uiNotifications=[];storageSet("dread_notifications","[]");renderNotifications();});var obs=new MutationObserver(function(){syncLobbyFocusMode();syncNotificationUi();});[document.getElementById("joinCodePanel"),document.getElementById("roomWaitPanel"),document.getElementById("authOverlay")].filter(Boolean).forEach(function(el){obs.observe(el,{attributes:true,attributeFilter:["class"]});});syncLobbyFocusMode();syncNotificationUi();renderNotifications();},0);'
  ].join("\n");
  const end=s.lastIndexOf('})();');
  if(end<0)throw new Error('v8.2 client end marker missing');
  s=s.slice(0,end)+"\n"+injected+"\n"+s.slice(end);
  assert(s,'wall:{wood:8,stone:3,scrap:2}','client wall price');
  assert(s,'STRUCTURE_MAX_LEVEL_UI','client structure levels');
  assert(s,'notificationEscape','notification center');
...
function patchHtml(){
  ...
  if(!s.includes('id="notificationBell"')){
